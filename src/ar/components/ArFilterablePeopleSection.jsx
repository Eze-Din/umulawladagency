  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 px-4 md:px-8 lg:px-16 py-8">
      ...
      <div className="w-full md:w-1/3 bg-gray-600 dark:bg-gray-700 p-4 rounded-lg shadow-lg">
        <h2 className="text-lg font-bold text-gold-600 dark:text-gold-400 mb-4">ترشح</h2>
        <Accordion title="عمر">
          {ageGroups.map((age) => (
            <div key={age} className="mb-2">
              <label className="inline-flex items-center">
                ...
                />
                <span className="ml-2 text-gold-600 dark:text-gold-400 font-bold">{age}</span>
              </label>
            </div>
          ))}
        </Accordion>
          ...
          {experiences.map((exp) => (
            <div key={exp} className="mb-2">
              <label className="inline-flex items-center">
                ...
                />
                <span className="ml-2 text-gold-600 dark:text-gold-400 font-bold">{exp}</span>
              </label>
            </div>
          ))}
        </Accordion>
          ...
          {languages.map((lang) => (
            <div key={lang} className="mb-2">
              <label className="inline-flex items-center">
                ...
                />
                <span className="ml-2 text-gold-600 dark:text-gold-400 font-bold">{lang}</span>
              </label>
            </div>
          ))}
        </Accordion>
          ...
          {religions.map((rel) => (
            <div key={rel} className="mb-2">
              <label className="inline-flex items-center">
                ...
                />
                <span className="ml-2 text-gold-600 dark:text-gold-400 font-bold">{rel}</span>
              </label>
            </div>
          ))}
        </Accordion>
  ...
  return (
    <div className="mb-4">
      ...
      >
        <span>{title}</span>
        <ChevronDownIcon
          className={`h-6 w-6 ${isOpen ? 'transform rotate-180' : ''} text-gray-900 dark:text-gray-100 transition-transform duration-300`}
          aria-hidden="true"
        />
      </button>
